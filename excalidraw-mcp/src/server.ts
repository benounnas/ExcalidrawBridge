...
export interface RegisterToolsOptions {
  onCreateView?: (elements: any[]) => void;
}

export function registerTools(server: McpServer, distDir: string, store: CheckpointStore, options?: RegisterToolsOptions): void {
...
Call read_me first to learn the element format.`,
    ...
    async ({ elements }): Promise<CallToolResult> => {
      ...
      options?.onCreateView?.(resolvedElements);
      return {
 ...
 */
export function createServer(store: CheckpointStore, options?: RegisterToolsOptions): McpServer {
